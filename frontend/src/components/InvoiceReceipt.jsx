...
const InvoiceReceipt = forwardRef(({ data }, ref) => {
    ...
    const _subtotal = Math.round(parseFloat(data.subtotal || data.total_amount || 0));
    const _taxAmt = Math.round(parseFloat(data.taxAmt !== undefined ? data.taxAmt : data.tax_amount || 0));
    const _discountAmt = Math.round(parseFloat(data.discountAmt !== undefined ? data.discountAmt : data.discount_amount || 0));
    const _total = Math.round(parseFloat(data.total !== undefined ? data.total : data.total_amount || 0));
    const _paidAmt = Math.round(parseFloat(data.paidAmt !== undefined ? data.paidAmt : data.amount_paid || 0));
    const _changeAmt = Math.round(parseFloat(data.change_amount || 0));
    const _remaining = Math.round(parseFloat(data.rem !== undefined ? data.rem : Math.max(0, _total - _paidAmt)));
                    ...
                    <tbody className="divide-y divide-dashed divide-black">
                        {items.map((item, idx) => (
                            <tr key={idx} className="border-b border-black border-dashed">
                                ...
                                <td className="py-2 px-1 text-right font-bold">{item.selling_price.toLocaleString()}</td>
                                <td className="py-2 px-1 text-right font-black">{item.subtotal.toLocaleString()}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
