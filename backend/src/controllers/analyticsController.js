    ...
    try {
        ...
        const statsArr = await db.sequelize.query(
            `SELECT 
                ...
                COALESCE(SUM(si.profit_total), 0) as gross_profit,
                COALESCE(SUM(si.cost_total), 0) as total_cost
             FROM sales s
             LEFT JOIN (
                 SELECT 
                    sale_id, 
                    SUM((selling_price - purchase_price) * quantity) as profit_total,
        ...
        const stats = (statsArr && statsArr.length > 0) ? statsArr[0] : { transactions_count: 0, total_revenue: 0, gross_profit: 0, total_cost: 0 };
        ...
        salesCollected.forEach(item => {
            ...
            totalSalesCollected += amount;
        });

        // We still track paymentsCollected for the breakdown/history if needed, 
        // but it's excluded from the "Actual Collected (Net)" of current revenue
        paymentsCollected.forEach(item => {
            const method = item.name || 'other';
            const amount = parseFloat(item.total_collected || 0);
            actualCollectedMap[method] = (actualCollectedMap[method] || 0) + amount;
        });
        ...
        const result = {
            summary: {
                ...
                total_revenue: Math.round(parseFloat(stats?.total_revenue || 0)),
                total_profit: Math.round(parseFloat(stats?.gross_profit || 0)),
                total_expenses: Math.round(totalExpenses),
                net_profit: Math.round(parseFloat(stats?.gross_profit || 0) - totalExpenses),
                
                // --- RECONCILIATION METRICS ---
                // Actual Collected (Net) = (Amount Paid for Sales in Period) - (Total Expenses)
                actual_collected: Math.round(Math.max(0, totalSalesCollected - totalExpenses))
            },
